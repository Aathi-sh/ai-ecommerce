...
// middleware.js - COMPLETE WORKING VERSION WITH ALL HELPERS
import { NextResponse } from 'next/server';
import { getToken } from 'next-auth/jwt';
...
// ALL these paths and their subpaths are COMPLETELY PUBLIC
 ...
 */
const isCatalogPublicPath = (pathname) => {
  // Check all catalog public prefixes
  for (const prefix of catalogPublicPrefixes) {
    if (pathname === prefix || pathname.startsWith(`${prefix}/`)) {
      return true;
    }
  }
 ...
 */
const isWhatsappPath = (pathname) => {
  return pathStartsWithAny(pathname, whatsappPrefixes);
};
...
export async function middleware(request) {
  ...
  // ===== 6. PUBLIC PATHS (with auth redirect logic) =====
  if (isPublicPath(pathname)) {
    // Redirect authenticated users away from auth pages
    const token = await getToken({ 
      req: request,
  ...
  // ===== 7. FROM HERE ON, AUTHENTICATION IS REQUIRED =====
