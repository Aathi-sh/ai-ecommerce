 ...
 * - Session state management
 * - FCM notification cleanup for admin users
 * - PUBLIC CATALOGUE PAGES (no authentication required)
 */
...
export function AuthProvider({ children }) {
  ...
  // ========== FIXED: Handle route protection with catalogue pages PUBLIC ==========
  useEffect(() => {
    if (loading || !authChecked) return;

    const handleRouteProtection = () => {
      // ===== CRITICAL: Catalogue pages are ALWAYS public =====
      // Check if this is a catalogue page FIRST
      const isCataloguePage = pathname?.startsWith('/catalogue');
      
      if (isCataloguePage) {
        console.log('📱 [AuthContext] Catalogue page - NO AUTH REQUIRED:', pathname);
        return; // EARLY RETURN - Skip all auth checks for catalogue pages
      }
      ...
      // For authenticated users, check status and roles
      if (isAuthenticated) {
        // Check user status
        if (user.status === 'pending') {
          console.log('🔄 [AuthContext] Redirecting pending verification user:', pathname);
        ...
        // Company status check (for non-super-admin)
        if (!user.isSuperAdmin && user.companyId) {
          // You can add company status check here if needed
        }

        // Role-based route access for active users
        if (user.status === 'active') {
          // Super admin routes
          if (pathname?.startsWith('/super-admin') && !user.isSuperAdmin) {
            console.log('⛔ [AuthContext] Non-super-admin accessing super admin route:', pathname);
            router.push('/admin/dashboards');
            return;
          }
          ...
          // Admin routes
          if (pathname?.startsWith('/admin') && !user.isAdmin) {
            console.log('⛔ [AuthContext] Non-admin accessing admin route:', pathname);
            router.push('/dashboard');
            return;
          }
          ...
          // Manager routes
          if (pathname?.startsWith('/manager') && !isManagerOrAdmin) {
            console.log('⛔ [AuthContext] Unauthorized user accessing manager route:', pathname);
            router.push('/dashboard');
            return;
          }
        }
  ...
  const contextValue = useMemo(() => ({
    ...
    canAccessRoute: (route) => {
      // Catalogue routes are always accessible
      if (route?.startsWith('/catalogue')) return true;
...
export function withAuth(Component, options = {}) {
  const { 
    ...
    requireActive = true,
    requireCompany = true,
    skipForCatalogue = true // NEW: Skip auth for catalogue pages
  } = options;
  ...
  return function WithAuthWrapper(props) {
    ...
    // Skip authentication for catalogue pages
    if (skipForCatalogue && pathname?.startsWith('/catalogue')) {
      return <Component {...props} />;
    }
