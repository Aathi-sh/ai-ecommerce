    ...
    try {
      ...
      if (data.success && data.data) {
        ...
        flatCategories.forEach(cat => {
          categoryMap.set(cat._id, { ...cat, subcategories: [] });
        });
        
        flatCategories.forEach(cat => {
          if (cat.parentId && categoryMap.has(cat.parentId)) {
            categoryMap.get(cat.parentId).subcategories.push(categoryMap.get(cat._id));
          } else if (!cat.parentId || cat.level === 0) {
            treeCategories.push(categoryMap.get(cat._id));
          }
        });
