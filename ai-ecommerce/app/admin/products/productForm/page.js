...
// // Image configuration - PROFESSIONAL STANDARDS
// const IMAGE_CONFIG = {
//     MAX_FILES: 8,
//     MAX_SIZE_MB: 2, // 2MB per image (professional standard)
//     MAX_SIZE_BYTES: 2 * 1024 * 1024,
//     MAX_TOTAL_MB: 8, // 8MB total across all images
...
//     const calculateTotalImageSize = useCallback(() => {
//         let total = 0;
        
//         // Add sizes of new image files
//         imageFiles.forEach(file => {
//             total += file.size;
//         });
        
//         // For existing images, we need to estimate (they're already uploaded)
//         // We'll assume existing images are already within limits
//         // But we still count them as part of the total (using a conservative estimate)
//         // We don't know their actual size, so we'll use the max allowed per image
//         // This is safe because they were already validated when uploaded
//         formData.imageUrls.forEach(() => {
//             total += IMAGE_CONFIG.MAX_SIZE_BYTES; // Conservative estimate
//         });
        
//         return total;
//     }, [imageFiles, formData.imageUrls]);

//     // Update total size whenever images change
//     useEffect(() => {
//         const total = calculateTotalImageSize();
...
//                 if (mains.length === 0) {
//                     showToast('warning', 'No categories found. Please create a category first.');
//                 }
...
//         if (totalImageSize > IMAGE_CONFIG.MAX_TOTAL_BYTES) {
//             const currentMB = (totalImageSize / (1024 * 1024)).toFixed(2);
//             newErrors.images = `Total images size (${currentMB}MB) exceeds the ${IMAGE_CONFIG.MAX_TOTAL_MB}MB limit. Please remove some images or use smaller images.`;
//         }
...
//         let currentTotalSize = totalImageSize;
        
//         const validFiles = files.filter(file => {
//             // Check file type
//             if (!IMAGE_CONFIG.ALLOWED_TYPES.includes(file.type)) {
//                 showToast('error', `Invalid file type: ${file.name}. Please upload JPEG, PNG, or WebP images.`);
//                 return false;
//             }
...
//                 showToast('error', `File too large: ${file.name} (${fileSizeMB}MB). Maximum size is ${IMAGE_CONFIG.MAX_SIZE_MB}MB per image. Please compress this image.`);
//                 return false;
//             }
...
//         if (newTotalSize > IMAGE_CONFIG.MAX_TOTAL_BYTES) {
//             const remainingMB = ((IMAGE_CONFIG.MAX_TOTAL_BYTES - currentTotalSize) / (1024 * 1024)).toFixed(2);
//             const requestedMB = (newFilesTotalSize / (1024 * 1024)).toFixed(2);
//             showToast('error', `Cannot add ${validFiles.length} image(s) (${requestedMB}MB). Total would exceed ${IMAGE_CONFIG.MAX_TOTAL_MB}MB limit. Only ${remainingMB}MB remaining.`);
//             return;
//         }

//         // Check remaining slots
//         const remainingSlots = IMAGE_CONFIG.MAX_FILES - imagePreviews.length;
//         const filesToAdd = validFiles.slice(0, remainingSlots);
        
//         if (filesToAdd.length === 0) {
//             showToast('error', `Maximum ${IMAGE_CONFIG.MAX_FILES} images allowed per product`);
//             return;
//         }
...
//                     const addedSizeMB = (filesToAdd.reduce((sum, f) => sum + f.size, 0) / (1024 * 1024)).toFixed(2);
//                     showToast('success', `${filesToAdd.length} image(s) added (${addedSizeMB}MB)`);
//                 }
//             };
...
//         // If removing a newly uploaded file (not from server)
...
//             try {
//                 showToast('info', `Uploading image ${i + 1} of ${imageFiles.length}...`);
...
//                     if (res.status === 413) {
//                         errorMessage = `Image too large: ${file.name}. Please compress to under ${IMAGE_CONFIG.MAX_SIZE_MB}MB.`;
//                     } else if (res.status === 400) {
//                         const errorData = await res.json().catch(() => ({}));
//                         errorMessage = errorData.message || `Invalid file: ${file.name}`;
//                     }
...
//             showToast('warning', "Please wait, upload in progress...");
//             return;
//         }
...
//     // Calculate remaining size for display
//     const remainingSizeMB = Math.max(0, (IMAGE_CONFIG.MAX_TOTAL_BYTES - totalImageSize) / (1024 * 1024)).toFixed(2);
//     const totalSizeMB = (totalImageSize / (1024 * 1024)).toFixed(2);
//     const isSizeLimitReached = totalImageSize >= IMAGE_CONFIG.MAX_TOTAL_BYTES;
...
//                                 disabled={saving || isSubmitting || uploadingImages}
//                                 className="save-button desktop-save"
//                             >
//                                 {saving || isSubmitting || uploadingImages ? (
//                                     <>
...
//                                             <span className="image-count">{imagePreviews.length}/{IMAGE_CONFIG.MAX_FILES} images · {totalSizeMB}/{IMAGE_CONFIG.MAX_TOTAL_MB}MB used</span>
//                                         </label>
                                        
//                                         {/* Size limit warning */}
//                                         {totalImageSize > 0 && (
//                                             <div className={`size-info ${isSizeLimitReached ? 'size-limit-reached' : ''}`}>
//                                                 <AlertCircle size={14} />
//                                                 <span>
//                                                     {isSizeLimitReached 
//                                                         ? `Size limit reached (${IMAGE_CONFIG.MAX_TOTAL_MB}MB max). Remove some images to add more.`
//                                                         : `${remainingSizeMB}MB remaining out of ${IMAGE_CONFIG.MAX_TOTAL_MB}MB total limit`
//                                                     }
//                                                 </span>
//                                             </div>
//                                         )}
...
//                                         {imagePreviews.length < IMAGE_CONFIG.MAX_FILES && !isSizeLimitReached && (
//                                             <div 
...
//                                                 <span>JPEG, PNG, WebP (Auto-compressed to ~800KB, max {IMAGE_CONFIG.MAX_TOTAL_MB}MB total)</span>
//                                             </div>
//                                         )}
...
//                                                 <Upload size={32} />
//                                                 <p>Size limit reached</p>
//                                                 <span>Remove some images to upload more (max {IMAGE_CONFIG.MAX_TOTAL_MB}MB total)</span>
//                                             </div>
//                                         )}
//                                     </div>
//                                 </div>
...
//                     border-left: 4px solid ${appTheme.colors.info};
//                 }
...
//                     border: 2px dashed ${appTheme.colors.error};
//                     border-radius: ${appTheme.radius.md};
//                     padding: 32px;
//                     text-align: center;
//                     background: ${appTheme.colors.error}05;
//                     opacity: 0.6;
//                 }
...
// ==================== IMAGE CONFIGURATION - UPDATED FOR AUTO-COMPRESSION ====================
// Backend now automatically compresses ANY file size to ~800KB
// Frontend only enforces count limit and total size limit (generous)
const IMAGE_CONFIG = {
    MAX_FILES: 8,                       // Max 8 images per product
    MAX_TOTAL_MB: 50,                   // 50MB total limit (backend will compress)
    MAX_TOTAL_BYTES: 50 * 1024 * 1024,  // 50MB in bytes
    // PER-FILE SIZE LIMIT REMOVED - Backend handles compression automatically
    ALLOWED_TYPES: ['image/jpeg', 'image/jpg', 'image/png', 'image/webp'],
    ALLOWED_EXTENSIONS: ['.jpg', '.jpeg', '.png', '.webp']
};
...
export default function ProductForm() {
    ...
    const [totalImageSize, setTotalImageSize] = useState(0);
    const [uploadingImages, setUploadingImages] = useState(false);
    // Separate state for existing server images and new uploads
    ...
    // Calculate total size of all images (existing + new)
    const calculateTotalImageSize = useCallback(() => {
        let total = 0;
        ...
        // For existing images, we use a conservative estimate (1MB each)
        // Since backend already compressed them, they're likely under 1MB
        existingImageUrls.forEach(() => {
            total += 1 * 1024 * 1024; // Assume 1MB per existing image
        });
    ...
    const validateForm = () => {
        ...
        // Check if any images exist (existing or new)
        if (existingImageUrls.length === 0 && newImagePreviews.length === 0) {
            newErrors.images = "At least one product image is required";
        }
    ...
    // UPDATED: Handle image selection - NO PER-FILE SIZE LIMIT
    const handleImageChange = (e) => {
        const files = Array.from(e.target.files);
        ...
        const validFiles = files.filter(file => {
            // Check file type only (size check removed - backend handles compression)
            if (!IMAGE_CONFIG.ALLOWED_TYPES.includes(file.type)) {
                showToast('error', `Invalid file type: ${file.name}. Please upload JPEG, PNG, or WebP images.`);
                return false;
            }
            
            // Show info if file is large (will be compressed by backend)
            const fileSizeMB = (file.size / (1024 * 1024)).toFixed(2);
            if (parseFloat(fileSizeMB) > 2) {
                showToast('info', `Large file detected: ${file.name} (${fileSizeMB}MB). Will be automatically compressed.`);
            }
        ...
        // Check remaining slots
        const totalCurrentImages = existingImageUrls.length + newImagePreviews.length;
        const remainingSlots = IMAGE_CONFIG.MAX_FILES - totalCurrentImages;
        const filesToAdd = validFiles.slice(0, remainingSlots);
        ...
        filesToAdd.forEach(file => {
            ...
            reader.onload = (e) => {
                ...
                if (newPreviews.length === filesToAdd.length) {
                    ...
                    const addedSizeMB = (filesToAdd.reduce((sum, f) => sum + f.size, 0) / (1024 * 1024)).toFixed(2);
                    showToast('success', `${filesToAdd.length} image(s) added (${addedSizeMB}MB) - will be compressed on upload`);
                }
            };
            reader.readAsDataURL(file);
        });
    ...
    // UPDATED: Remove image (handles both existing and new)
    const removeImage = (index, isExisting = false) => {
        if (isExisting) {
            ...
            // Remove newly uploaded image (not yet on server)
    ...
    const getAllImages = () => {
        return {
            ...
            total: existingImageUrls.length + newImagePreviews.length
        };
    };
    ...
    const uploadImages = async () => {
        ...
        for (let i = 0; i < newImageFiles.length; i++) {
            ...
            try {
                showToast('info', `Uploading image ${i + 1} of ${newImageFiles.length}...`);
                ...
                if (!res.ok) {
                    ...
                    if (res.status === 413) {
                        errorMessage = `File too large: ${file.name}. Backend will compress automatically.`;
                    } else if (res.status === 400) {
                        const errorData = await res.json().catch(() => ({}));
                        errorMessage = errorData.message || `Invalid file: ${file.name}`;
                    }
                ...
                if (data.success) {
                    ...
                    if (data.metadata && data.metadata.savedPercent > 0) {
                        showToast('success', `Uploaded: ${file.name} (Saved ${data.metadata.savedPercent}%)`);
                    } else {
                        showToast('success', `Uploaded: ${file.name}`);
                    }
                } else {
    ...
    const totalCurrentImages = images.total;
    const remainingSlots = IMAGE_CONFIG.MAX_FILES - totalCurrentImages;
    const remainingSizeMB = Math.max(0, (IMAGE_CONFIG.MAX_TOTAL_BYTES - totalImageSize) / (1024 * 1024)).toFixed(2);
    const totalSizeMB = (totalImageSize / (1024 * 1024)).toFixed(2);
    const isSizeLimitReached = totalImageSize >= IMAGE_CONFIG.MAX_TOTAL_BYTES;
            ...
            <div className="product-form-page">
                ...
                <main className="main-content">
                    ...
                    <div className="form-sections">
                        ...
                        {/* Media Section - UPDATED with auto-compression info */}
                        <div className="form-section">
                            <div className="section-header">
                                <div className="section-header-left">
                                    <div className="section-icon" style={{ background: `${appTheme.colors.warning}15`, color: appTheme.colors.warning }}>
                                    ...
                                    <div>
                                        <h2>Media</h2>
                                        <p>Product images and videos - Large images auto-compressed to ~800KB</p>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="form-row">
                                    <div className="form-group span-3">
                                        <label>
                                            ...
                                            <span className="image-count">{totalCurrentImages}/{IMAGE_CONFIG.MAX_FILES} images · {totalSizeMB}/{IMAGE_CONFIG.MAX_TOTAL_MB}MB used</span>
                                        </label>
                                        
                                        {/* Auto-compression info banner */}
                                        <div className="compression-info">
                                            <Zap size={14} />
                                            <span>✨ Automatic compression: Any size image will be optimized to ~800KB (WebP format)</span>
                                        </div>
                                            ...
                                            <div className="image-grid">
                                                {existingImageUrls.map((url, index) => (
                                                    <div key={`existing-${index}`} className="image-preview">
                                                        <img src={url} alt={`Existing product ${index + 1}`} />
                                                        <button
                                                            type="button"
                                        ...
                                        {remainingSlots > 0 && !isSizeLimitReached && (
                                            <div 
                                                className="upload-area"
                                                onClick={() => document.getElementById("image-upload").click()}
                                                onDragOver={(e) => e.preventDefault()}
