...
export default function DashboardPage() {
  ...
  // Fetch all data with proper error handling and authentication
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      // Fetch users with authentication (changed from customers)
      try {
        const usersRes = await fetch("/api/users", { headers });
        if (usersRes.ok) {
          const usersData = await usersRes.json();
          if (usersData.success) {
            // Access users from data.users (correct API response structure)
        ...
        } else if (usersRes.status === 404 || usersRes.status === 401) {
          console.warn("Users API not accessible:", usersRes.status);
          // Extract users from orders as fallback
          const customerEmails = new Set();
          orders.forEach(order => {
            if (order.email) customerEmails.add(order.email);
          });
  ...
  if (error) {
    return (
      <div style={{ 
        ...
        <h2 style={{ color: appTheme.colors.error }}>Authentication Required</h2>
