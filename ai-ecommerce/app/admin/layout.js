...
import { useRouter } from "next/navigation";
import Head from "next/head";
import LogoSplash from "../../src/components/logoSplash"; // ✅ Import the splash component
...
export default function AdminLayout({ children }) {
  ...
  // ==================== AUTO-INITIALIZATION ====================
  useEffect(() => {
    // ✅ Only proceed if user is authenticated, active, and admin AND splash is done
    if (isClient && user && isActive && hasRole('admin') && !authLoading && !showSplash) {
      if (Notification.permission === 'default' && !notificationPermissionRequestedRef.current) {
        notificationPermissionRequestedRef.current = true;
        setTimeout(() => {
          handleRequestNotificationPermission();
  ...
  }, [isClient, user, authLoading, handleRequestNotificationPermission, isActive, hasRole, showSplash]);
  ...
  // ✅ Show splash screen first (before any authentication checks)
  if (showSplash) {
