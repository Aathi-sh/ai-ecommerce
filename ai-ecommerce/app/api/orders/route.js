  ...
  try {
    ...
    if (id) {
      const order = await Order.findById(id).populate({
        ...
        model: Product,
        select: "productName price imageUrls category stock" // Added stock
      });
...
export async function POST(request) {
  try {
    ...
    // 🔥 NEW: STOCK VALIDATION - Check stock availability for all items
    ...
    // 🔥 NEW: UPDATE STOCK AFTER SUCCESSFUL ORDER CREATION
    ...
    for (const item of body.items) {
      try {
        ...
        if (updatedProduct) {
          console.log(`✅ Stock updated: ${updatedProduct.productName} - New stock: ${updatedProduct.stock} (Reduced by: ${item.quantity})`);
          
          // Optional: Check for low stock alert
...
export async function PUT(request) {
  try {
    ...
    if (updateData.status) {
      ...
      // 🔥 NEW: HANDLE STOCK RESTORATION WHEN ORDER IS CANCELLED
          ...
          try {
            ...
            if (product) {
              const updatedProduct = await Product.findByIdAndUpdate(
                item.productId,
                { 
                  $inc: { stock: item.quantity }, // Increase stock atomically
                  updatedAt: new Date()
                },
                { new: true }
              );
              ...
              if (updatedProduct) {
                console.log(`↩️ Stock restored: ${product.productName} - New stock: ${updatedProduct.stock} (Added back: ${item.quantity})`);
              }
            }
          } catch (stockError) {
      ...
      // 🔥 NEW: HANDLE STOCK DEDUCTION WHEN ORDER MOVES FROM CANCELLED TO ACTIVE
      if (currentOrder.status === 'cancelled' && 
          updateData.status !== 'cancelled' && 
          updateData.status !== 'pending') {
        console.log("🔄 Order reactivated - Reducing stock...");
        ...
        for (const item of currentOrder.items) {
          try {
            ...
            if (product && product.stock >= item.quantity) {
              ...
              if (updatedProduct) {
                console.log(`📦 Stock reduced: ${product.productName} - New stock: ${updatedProduct.stock} (Reduced by: ${item.quantity})`);
              }
            } else if (product) {
              console.warn(`⚠️ Cannot reactivate order: Insufficient stock for ${product.productName}`);
            }
          } catch (stockError) {
...
export async function DELETE(request) {
  try {
    ...
    // 🔥 NEW: RESTORE STOCK BEFORE DELETING ORDER
      ...
      for (const item of orderToDelete.items) {
        try {
          ...
          if (product) {
            ...
            if (updatedProduct) {
              console.log(`↩️ Stock restored on delete: ${product.productName} - New stock: ${updatedProduct.stock}`);
            }
          }
        } catch (stockError) {
          console.error(`❌ Error restoring stock on delete for product ${item.productId}:`, stockError);
        }
      }
    }
