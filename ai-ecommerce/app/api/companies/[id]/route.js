...
// Format company response with WhatsApp fields and catalog fields
...
export async function PUT(request, { params }) {
  try {
    ...
    // ✅ NEW: Validate slug if being updated
    if (body.slug !== undefined) {
      if (!body.slug?.trim()) {
        errors.slug = 'Slug cannot be empty';
      } else if (!validateSlug(body.slug)) {
        errors.slug = 'Slug can only contain lowercase letters, numbers, and hyphens';
      }
    }

    // ✅ NEW: Validate catalog WhatsApp if being updated
    if (body.catalogWhatsapp !== undefined && body.catalogWhatsapp && !validatePhone(body.catalogWhatsapp)) {
    ...
    // Check for duplicate email/name/slug if updating
    if (body.companyEmail || body.companyName || body.slug) {
      ...
      if (body.slug) {
        duplicateQuery.slug = body.slug.toLowerCase().trim();
      }
      ...
      if (existing) {
        let field = '';
        if (existing.companyEmail === body.companyEmail?.toLowerCase().trim()) {
          field = 'companyEmail';
        } else if (existing.companyName === body.companyName?.trim()) {
          field = 'companyName';
        } else if (existing.slug === body.slug?.toLowerCase().trim()) {
          field = 'slug';
        }
    ...
    console.log('✅ [COMPANY DETAILS API] PUT successful:', {
      ...
      catalogWhatsapp: updatedCompany.catalogWhatsapp,
    });
...
export async function PATCH(request, { params }) {
  try {
    ...
    const updatedCompany = await Company.findById(params.id)
      .select('_id companyName slug catalogWhatsapp whatsapp')
      .lean();
...
// // app/api/companies/[id]/route.js
// // PROFESSIONAL COMPANY DETAILS API - Full WhatsApp multi-tenant support with Service Type Management
// // Handles: Get, Update, Patch (WhatsApp status), Delete company
...
//               isEcommerce: company.serviceType === 'ecommerce' || company.serviceType === 'both',
//               isBooking: company.serviceType === 'booking' || company.serviceType === 'both',
//               enabledModules: company.serviceType === 'ecommerce' ? ['ecommerce'] : 
//                               company.serviceType === 'booking' ? ['booking'] : 
//                               ['ecommerce', 'booking']
//             }
//           },
//           timestamp: new Date().toISOString(),
...
//                             company.serviceType === 'booking' ? ['booking'] : 
//                             ['ecommerce', 'booking'],
//             history: company.serviceTypeHistory || []
//           }
//         },
//         timestamp: new Date().toISOString(),
//       },
...
//     if (body.serviceType !== undefined && !validateServiceType(body.serviceType)) {
//       errors.serviceType = 'Service type must be ecommerce, booking, or both';
//     }

//     // ✅ Validate catalog WhatsApp if being updated
//     if (body.catalogWhatsapp !== undefined && body.catalogWhatsapp && !validatePhone(body.catalogWhatsapp)) {
//       errors.catalogWhatsapp = 'Catalog WhatsApp number must be 10-12 digits';
//     }
...
//     // ✅ Handle service type change using the model's method
//     if (body.serviceType && body.serviceType !== existingCompany.serviceType) {
//       try {
//         await existingCompany.updateServiceType(
//           body.serviceType, 
//           session.user.id, 
//           body.serviceTypeChangeReason || 'Service type updated via API'
//         );
//         // Remove serviceType from updateData to avoid double update
//         delete body.serviceType;
//       } catch (serviceTypeError) {
//         console.error('Error updating service type:', serviceTypeError);
//         return NextResponse.json(
//           {
//             success: false,
//             message: serviceTypeError.message,
//             code: 'SERVICE_TYPE_UPDATE_FAILED',
//           },
//           {
//             status: 400,
//             headers: { ...securityHeaders, ...corsHeaders },
...
//     if (body.plan && body.plan !== existingCompany.subscription?.plan) {
//       const planLimits = getPlanLimits(body.plan);
//       const planFeatures = getPlanFeatures(body.plan, existingCompany.serviceType);
...
//       companyName: updatedCompany.companyName,
//       slug: updatedCompany.slug,
//       serviceType: updatedCompany.serviceType,
//       catalogWhatsapp: updatedCompany.catalogWhatsapp,
//     });
...
//     // Get updated company with populated fields
//     const updatedCompany = await Company.findById(params.id)
//       .select('_id companyName slug catalogWhatsapp serviceType whatsapp')
//       .lean();
