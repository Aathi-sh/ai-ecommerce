...
import Professional from '@/models/Bookingmng'; // Your model name
...
export async function GET(request) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to fetch professionals' },
      { status: 500 }
    );
  }
}
...
export async function PATCH(request) {
  try {
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to update professional' },
      { status: 500 }
    );
  }
}
...
export async function DELETE(request) {
  try {
    ...
    await User.findByIdAndUpdate(professional.userId, {
      $set: { role: 'user', professionalId: null }
    });
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Failed to delete professional' },
      { status: 500 }
    );
  }
}