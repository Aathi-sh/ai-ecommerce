...
// above code is workig wihout saas
...
export async function POST(request) {
  try {
    ...
    const user = await User.findOne({
      ...
      resetPasswordExpires: { $gt: Date.now() },
      status: 'active',
    }).select('+password').populate('companyId', 'companyName status subscription');
    ...
    if (!isSuperAdmin && user.companyId) {
      ...
      if (!company) {
        console.log("⚠️ [RESET-PASSWORD API] User has invalid company:", user.email);
        return NextResponse.json(
          {
            success: false,
            message: "Your account is not properly configured. Please contact support.",
            code: "COMPANY_MISCONFIGURED",
          },
          { status: 403 }
        );
      }
      ...
      if (company.status !== 'active') {
        console.log("⚠️ [RESET-PASSWORD API] Company not active:", {
          email: user.email,
      ...
      if (company.subscription?.expiryDate && new Date(company.subscription.expiryDate) < new Date()) {
        console.log("⚠️ [RESET-PASSWORD API] Company subscription expired:", company.companyName);
    ...
    // Direct MongoDB update - COMPLETELY BYPASS Mongoose middleware
    const updateResult = await User.updateOne(
      { _id: user._id },
      {
        $set: {
    ...
    // ===== SAAS: UPDATE COMPANY LAST ACTIVE =====
    if (user.companyId && !isSuperAdmin) {
      try {
        ...
        console.log("✅ [RESET-PASSWORD API] Company lastActive updated for:", user.companyId.companyName);
      } catch (companyError) {
        console.warn("⚠️ [RESET-PASSWORD API] Failed to update company lastActive:", companyError.message);
      }
    }

    // Send confirmation email with company context (non-blocking)
    try {
      const { sendEmail } = await import('@/utils/email');
      sendEmail({
        to: user.email,
...
© ${new Date().getFullYear()} Steponext. All rights reserved.`,
    ...
    // Log the password reset for security audit with company context
    console.log("📝 [RESET-PASSWORD API] Password reset completed:", {
      email: user.email,
      userId: user._id.toString(),
      role: user.role,
...
export async function GET() {
  return NextResponse.json({
    endpoint: "/api/auth/reset-password",
    method: "POST",
    description: "Multi-tenant password reset endpoint",
    required_fields: {
      token: "string (64-character hex token)",
      password: "string (min 6 characters, with uppercase, lowercase, numbers)"
    },
    ...
    saas_features: [
      "Company status validation before reset",
