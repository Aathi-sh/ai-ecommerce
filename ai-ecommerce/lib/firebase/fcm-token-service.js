// // // lib/firebase/fcm-token-service.js
// // import { getToken, onMessage, deleteToken } from 'firebase/messaging';
// // import { messaging, initializeFirebase } from './firebase-client';
// // import getWebSocketClient from '../websocket/socketio-client';
...
// // // THROTTLING VARIABLES - CRITICAL FIX
...
// // // ==================== FIXED: SAVE TOKEN WITH THROTTLING AND WEBSOCKET ====================
// // /**
// //  * Save FCM token to backend - WITH THROTTLING FIX AND WEBSOCKET INTEGRATION
// //  */
...
// //     // ✅ NEW: Send token to WebSocket server for immediate use
// //     try {
// //       const wsClient = getWebSocketClient();
// //       const wsStatus = wsClient.getStatus();
      
// //       if (wsStatus.isConnected) {
// //         console.log('🔗 Sending token to WebSocket server...');
// //         wsClient.registerFCMToken(token);
// //         console.log('✅ Token sent to WebSocket server');
// //       } else {
// //         console.warn('⚠️ WebSocket not connected, will send token when connected');
// //         // Store token for later WebSocket registration
// //         if (typeof window !== 'undefined') {
// //           const pendingWebSocketTokens = JSON.parse(localStorage.getItem('pending_ws_tokens') || '[]');
// //           pendingWebSocketTokens.push({
// //             token,
// //             timestamp: Date.now(),
// //           });
// //           localStorage.setItem('pending_ws_tokens', JSON.stringify(pendingWebSocketTokens.slice(-3))); // Keep only last 3
// //         }
// //       }
// //     } catch (wsError) {
// //       console.warn('⚠️ Could not send token to WebSocket:', wsError.message);
...
// //     console.log('✅ FCM token saved to backend and WebSocket server!');
...
// // /**
// //  * Process pending FCM tokens (including WebSocket tokens)
// //  */
...
// //     const pendingWebSocketTokensStr = localStorage.getItem('pending_ws_tokens');
    
// //     if (!pendingTokensStr && !pendingWebSocketTokensStr) {
...
// //         if (successful > 0) {
// //           const remainingTokens = pendingTokens.slice(successful);
...
// //     // Process WebSocket pending tokens
// //     if (pendingWebSocketTokensStr) {
// //       const pendingWebSocketTokens = JSON.parse(pendingWebSocketTokensStr);
// //       if (Array.isArray(pendingWebSocketTokens) && pendingWebSocketTokens.length > 0) {
// //         const wsClient = getWebSocketClient();
// //         const wsStatus = wsClient.getStatus();
        
// //         if (wsStatus.isConnected) {
// //           for (const pending of pendingWebSocketTokens) {
// //             try {
// //               wsClient.registerFCMToken(pending.token);
// //               successful++;
// //             } catch (error) {
// //               failed++;
// //               console.error('Failed to send token to WebSocket:', error.message);
// //             }
// //           }
          
// //           // Clear WebSocket tokens
// //           localStorage.removeItem('pending_ws_tokens');
// //         }
// //       }
// //     }
    
// //     console.log(`✅ Processed ${successful} tokens, ${failed} failed`);
// //     return { success: true, processed: successful, failed, total: successful + failed };
// //   } catch (error) {
...
// // /**
// //  * Send token to WebSocket server directly
// //  */
// // export const sendTokenToWebSocketServer = async (token) => {
// //   try {
// //     if (!token || typeof token !== 'string') {
...
// //       console.log('📡 Sending token to WebSocket server...');
// //       const success = wsClient.registerFCMToken(token);
      
// //       if (success) {
// //         console.log('✅ Token sent to WebSocket server successfully');
// //         return { success: true, message: 'Token sent to WebSocket server' };
// //       } else {
// //         throw new Error('Failed to send token to WebSocket server');
// //       }
// //     } else {
// //       console.warn('⚠️ WebSocket not connected, storing token for later');
      
// //       // Store for later
// //       if (typeof window !== 'undefined') {
// //         const pendingTokens = JSON.parse(localStorage.getItem('pending_ws_tokens') || '[]');
// //         pendingTokens.push({
// //           token,
// //           timestamp: Date.now(),
// //         });
// //         localStorage.setItem('pending_ws_tokens', JSON.stringify(pendingTokens.slice(-3)));
// //       }
      
// //       return { 
// //         success: false, 
// //         message: 'WebSocket not connected, token will be sent when connected',
// //         storedForLater: true
// //       };
// //     }
// //   } catch (error) {
// //     console.error('❌ Error sending token to WebSocket:', error);
// //     return { success: false, error: error.message };
// //   }
// // };
...
// //     console.log('✅ FCM token obtained, saving to backend and WebSocket...');
...
//         // Send to WebSocket server even if already exists in database
//         await sendTokenToWebSocketIfPossible(token);
...
// /**
//  * Helper function to send token to WebSocket if possible
//  */
// const sendTokenToWebSocketIfPossible = async (token) => {
//   try {
//     const wsClient = getWebSocketClient();
//     const wsStatus = wsClient.getStatus();
...
//       if (typeof window !== 'undefined') {
//         const pendingWebSocketTokens = JSON.parse(localStorage.getItem('pending_ws_tokens') || '[]');
//         const existingToken = pendingWebSocketTokens.find(t => t.token === token);
//         if (!existingToken) {
//           pendingWebSocketTokens.push({
//             token,
//             timestamp: Date.now(),
...
//       if (typeof window !== 'undefined') {
//         const pendingTokens = JSON.parse(localStorage.getItem('pending_ws_tokens') || '[]');
//         const existingToken = pendingTokens.find(t => t.token === token);
//         if (!existingToken) {
//           pendingTokens.push({
//             token,
//             timestamp: Date.now(),
