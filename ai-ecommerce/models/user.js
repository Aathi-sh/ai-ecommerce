...
const userSchema = new mongoose.Schema(
  {
    ...
    role: {
      ...
      default: "admin", // ALWAYS DEFAULT TO ADMIN
    },
    
    // ... [ALL YOUR EXISTING NOTIFICATION SETTINGS CODE - KEEP AS IS] ...
    // ... [ALL YOUR EXISTING ADMIN PREFERENCES CODE - KEEP AS IS] ...
    // ... [ALL YOUR EXISTING SESSION MANAGEMENT CODE - KEEP AS IS] ...
    // ... [ALL YOUR EXISTING VERIFICATION & SECURITY CODE - KEEP AS IS] ...
    // ... [ALL YOUR EXISTING ACTIVITY TRACKING CODE - KEEP AS IS] ...
    // ... [ALL YOUR EXISTING AUDIT FIELDS CODE - KEEP AS IS] ...
    // ... [ALL YOUR EXISTING STATUS & META CODE - KEEP AS IS] ...
    // ... [ALL YOUR EXISTING PERFORMANCE METRICS CODE - KEEP AS IS] ...
  },
  { 
    timestamps: true,
    toJSON: { virtuals: true },
...
userSchema.index({ "notificationSettings.pushNotifications.enabled": 1 });
userSchema.index({ role: 1, "notificationSettings.pushNotifications.enabled": 1 });
// ADD THIS FOR SESSION CLEANUP PERFORMANCE:
userSchema.index({ "activeSessions.expiresAt": 1 });

// ========== VIRTUAL PROPERTIES ==========
userSchema.virtual('isAdmin').get(function() {
  return this.role === 'admin'; // Will always return true
});
...
userSchema.pre("save", async function (next) {
  ...
  // Track login count when lastLogin is updated
  if (this.isModified('lastLogin') && this.lastLogin) {
    this.loginCount = (this.loginCount || 0) + 1;
  }
...
// ========== NOTIFICATION SETTINGS METHODS ==========
// ... [ALL YOUR EXISTING NOTIFICATION METHODS - KEEP AS IS] ...

// ========== SESSION MANAGEMENT METHODS ==========
// ... [ALL YOUR EXISTING SESSION METHODS - KEEP AS IS] ...

// ========== STATIC METHODS ==========
// ... [ALL YOUR EXISTING STATIC METHODS - KEEP AS IS] ...
...
// Find users with notification enabled
userSchema.statics.findUsersWithNotificationsEnabled = function() {
  return this.find({
    status: 'active',
    'notificationSettings.pushNotifications.enabled': true,
  }).select('fullName email phone notificationSettings adminPreferences');
