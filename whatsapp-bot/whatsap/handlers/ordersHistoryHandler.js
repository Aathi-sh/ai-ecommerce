...
// handlers/orderHistoryHandler.js - PROFESSIONAL MULTI-TENANT VERSION
// Handles order history with company isolation and proper customer identification

import apiService from "../../services/apiService.js";

// Helper function to extract clean phone number from WhatsApp message
...
// Helper function to format Indian phone numbers (for display)
function formatIndianPhoneNumber(phoneNumber) {
    if (!phoneNumber) return 'Unknown';
...
/**
 * MAIN FUNCTION: Handle order history request with company context
 * @param {Object} message - WhatsApp message object
 * @param {Object} client - WhatsApp client
 * @param {string} companyId - Company ID for multi-tenant isolation
 */
export async function handleOrdersHistory(message, client, companyId = null) {
    try {
        const from = message.from;
        
        // ✅ Use consistent phone extraction
        ...
        // 🔄 Use API call to get customer orders WITH company filter
        console.log(`🔍 Calling API for orders with phone: ${customerPhone} and company: ${companyId}`);
        const orders = await apiService.getCustomerOrders(customerPhone, companyId);
...
/**
 * Get order by ID with company validation
 * @param {Object} message - WhatsApp message
 * @param {string} orderId - Order ID
 * @param {string} companyId - Company ID for validation
 */
export async function getOrderById(message, orderId, companyId = null) {
    try {
        if (!orderId) {
            return await message.reply('❌ Please provide an order ID.');
        }

        const order = await apiService.getOrderById(orderId, companyId);
        
        if (!order) {
            return await message.reply(`❌ Order not found with ID: ${orderId}`);
        }

        // Verify order belongs to the same company (if companyId provided)
        if (companyId && order.companyId && order.companyId.toString() !== companyId.toString()) {
            return await message.reply(`❌ You don't have permission to view this order.`);
        }
...
/**
 * Get order statistics for admin (with company filter)
 * @param {Object} message - WhatsApp message
 * @param {string} companyId - Company ID for filtering
 */
export async function getOrderStats(message, companyId = null) {
    try {
        const stats = await apiService.getOrderStats('month', companyId);
