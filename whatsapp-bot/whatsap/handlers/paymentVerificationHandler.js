...
const VALIDATION_CONFIG = {
    ...
    recentPaymentThreshold: 15, // 15 minutes window
    duplicateWindow: 5 * 60 * 1000, // 5 minutes
    autoVerifyThreshold: 90, // Auto-verify if confidence ≥ 90%
    backupThreshold: 70, // Use backup engine if confidence 70-89%
    manualThreshold: 50, // Manual review if confidence < 50%
    cacheTTL: 30 * 60 * 1000, // 30 minutes
    companyUpiCacheTime: 5 * 60 * 1000 // 5 minutes
};
...
/**
 * Get company UPI IDs from cache/database
 */
async function getCompanyUpiIds(companyId) {
    try {
        // Check cache first
        const cached = companyUpiCache.get(companyId);
        if (cached && Date.now() - cached.timestamp < VALIDATION_CONFIG.companyUpiCacheTime) {
            return cached.upiIds;
        }

        // Fetch from database
        const activeUpiIds = await CompanyConfig.getActiveUpiIds(companyId);
        
        // Update cache
        companyUpiCache.set(companyId, {
            upiIds: activeUpiIds,
            timestamp: Date.now()
        });
        ...
        console.error(`❌ Failed to get UPI IDs for company ${companyId}:`, error);
        return [];
    }
}
...
export async function handlePaymentVerification(message, client) {
    try {
        ...
        // Handle menu or clear commands
        if (userMessage.toLowerCase() === '!menu' || userMessage.toLowerCase() === 'menu') {
            verificationState.delete(userPhone);
            userOrderState.delete(userPhone);
            await message.reply('🔄 *Starting fresh order process...*\n\nPlease wait for the menu...');
            return;
        ...
        if (userMessage.toLowerCase() === '!clear') {
            verificationState.delete(userPhone);
            userOrderState.delete(userPhone);
            await message.reply('✅ Verification state cleared. You can now send payment details.');
            return;
        }
...
async function handlePaymentMedia(message, client, userPhone) {
    try {
        ...
        // Check for duplicate processing
        const processingKey = `${userPhone}_${Date.now()}`;
        if (processedImages.has(processingKey)) {
            await message.reply('⏳ Please wait, your previous submission is still being processed.');
            return;
        }
        processedImages.set(processingKey, true);
        setTimeout(() => processedImages.delete(processingKey), VALIDATION_CONFIG.duplicateWindow);
        ...
        if (imageHashCache.has(imageHash)) {
            const lastUpload = imageHashCache.get(imageHash);
            if (Date.now() - lastUpload < VALIDATION_CONFIG.duplicateWindow) {
                return await message.reply('⏳ This image was already uploaded recently. Please wait for processing.');
            }
        }
        imageHashCache.set(imageHash, Date.now());
        ...
        // Get company UPI IDs
        const companyUpiIds = await getCompanyUpiIds(companyId);
        
        // Process with QR processor (handles both QR and screenshot detection)
        await message.reply('🔍 *Analyzing your payment...*\n\nPlease wait while we verify your payment details.');

        const paymentResult = await qrProcessor.processPayment({
            companyId,
            customerPhone: userPhone,
            image: Buffer.from(media.data, 'base64'),
            order: pendingOrders[0], // Primary order to check against
        ...
        if (paymentResult.decision?.action === 'auto_verify' || paymentResult.decision?.action === 'backup_verified') {
            await processSuccessfulVerification(
                paymentResult,
                pendingOrders.find(o => o.totalPrice === paymentResult.extractedFields?.amount),
                client,
                message,
                media.data,
                paymentResult,
        ...
        else if (paymentResult.type === 'qr_code' && paymentResult.extractedFields?.amount) {
            // QR code with amount - try to match with order
            const matchedOrder = pendingOrders.find(o => 
                Math.abs(o.totalPrice - paymentResult.extractedFields.amount) <= VALIDATION_CONFIG.amountTolerance
            );
            
            if (matchedOrder && paymentResult.validation?.upiMatch) {
                await processSuccessfulVerification(
                    paymentResult,
                    matchedOrder,
                    client,
                ...
                await handleFailedVerification({
                    ...
                    errors: paymentResult.validation?.upiMatch ? [] : ['UPI ID not recognized'],
                    warnings: []
                }, client, message);
            }
        }
            ...
            await handleFailedVerification({
                ...paymentResult,
                matchedOrder: pendingOrders[0],
                errors: paymentResult.errors || ['Could not verify payment'],
        ...
        await sendErrorMessage(message, '❌ Failed to process payment. Please try again or contact support.');
    }
}
...
async function handleTextPayment(message, client, userPhone, text) {
    try {
        ...
        // Process with QR processor (text version)
        const paymentResult = await qrProcessor.processPayment({
            companyId,
            customerPhone: userPhone,
            message: text,
        ...
        if (paymentResult.type === 'upi_text' && paymentResult.validation?.upiMatch) {
            // Valid UPI ID found
            await message.reply(`✅ *UPI ID Verified!*\n\nPlease send the payment screenshot to complete verification.`);
        } 
        else if (paymentResult.type === 'phone_number') {
            // Phone number provided
            await message.reply(`📱 *Phone Number Received*\n\nPlease send the payment screenshot to complete verification.`);
        }
        else {
            await message.reply(
                `❌ *Could Not Process*\n\n` +
                ...
                `• Payment screenshot\n` +
                `• QR code\n` +
                `• Or type !paymenthelp for instructions`
            );
        }

    } catch (error) {
        console.error('❌ Text payment error:', error);
        await sendErrorMessage(message, '❌ Failed to process payment information.');
    }
}
...
async function getCustomerPendingOrders(customerPhone) {
    try {
        ...
        const pendingOrders = allOrders.filter(order => {
            ...
            return isPaymentNotPaid && isOrderActive;
        });
 ...
 * Process successful verification
 */
async function processSuccessfulVerification(
    verificationResult, 
    order, 
    client, 
    originalMessage, 
    ...
    try {
        if (!order) {
            ...
            if (!order) {
                throw new Error('Could not match payment to any order');
            }
        }
        ...
        const paymentVerification = await apiService.createPaymentVerification({
            ...
            ocrAnalysis: {
                extractedText: verificationResult.text || ocrResult?.text || '',
                confidenceScore: verificationResult.confidence,
                extractedAmount: verificationResult.extractedFields?.amount,
                extractedUPI: verificationResult.extractedFields?.upiId,
                transactionId: verificationResult.extractedFields?.transactionId,
                timestamp: verificationResult.extractedFields?.timestamp,
                wordCount: verificationResult.wordCount,
                processingTime: verificationResult.metadata?.processingTime
            },
            detectedPayment: {
                amount: verificationResult.extractedFields?.amount,
                upiId: verificationResult.extractedFields?.upiId,
                transactionId: verificationResult.extractedFields?.transactionId,
                transactionTime: new Date(),
                status: 'success',
                appName: verificationResult.extractedFields?.appName,
                bankName: verificationResult.extractedFields?.bankName
            },
            validationResults: {
                amountMatch: verificationResult.validation?.amountMatch || false,
                upiMatch: verificationResult.validation?.upiMatch || false,
                confidenceScore: verificationResult.confidence,
                matchQuality: verificationResult.matchQuality || 'unknown',
                amountDifference: verificationResult.validation?.amountDifference,
                matchedUpiId: verificationResult.validation?.matchedUpiId
            },
            fraudAnalysis: {
                riskLevel: verificationResult.confidence >= 90 ? 'low' : 
                          verificationResult.confidence >= 70 ? 'medium' : 'high',
                fraudScore: 100 - verificationResult.confidence,
                isSuspicious: verificationResult.confidence < 70
            },
            metadata: {
                ocrEngine: verificationResult.engineUsed || verificationResult.type,
                backupUsed: verificationResult.backupUsed || false,
                processingTime: verificationResult.metadata?.processingTime,
                paymentType: verificationResult.type,
                companyId
            },
            status: 'verified',
            verifiedBy: 'auto_ocr',
        ...
        await originalMessage.reply(
            '✅ *PAYMENT RECEIVED!*\n\n' +
            'Your payment has been received. Our team will verify and process your order shortly.\n\n' +
            'If you have any questions, please contact support.'
        );
    }
}
...
async function handleFailedVerification(verificationResult, client, originalMessage) {
    ...
    errorMessage += `*Verification Score:* ${Math.round(verificationResult.confidence || 0)}%\n`;
    errorMessage += `*Minimum Required:* ${VALIDATION_CONFIG.minConfidenceScore}%\n\n`;
    ...
    errorMessage += `*How to Fix:*\n` +
        `1. Pay exactly ₹${order?.totalPrice || 'the order amount'}\n` +
        `2. Take clear screenshot showing:\n` +
        `   • Payment amount\n` +
        `   • "SUCCESSFUL" status\n` +
        `   • Transaction ID\n` +
        `   • Date & Time\n` +
        `3. Send screenshot immediately\n\n` +
        `Try again with correct payment details.`;
...
async function sendSuccessMessages(order, verificationResult, message, productNames) {
    const confidence = Math.round(verificationResult.confidence || 0);
    const engine = verificationResult.engineUsed || verificationResult.type || 'OCR';
    ...
    let successMessage = 
        ...
        `📦 *Product:* ${productNames}\n` +
        `💰 *Amount Paid:* ₹${order.totalPrice}\n` +
        `🎯 *Verification Score:* ${confidence}% (${engine})\n` +
        `🔢 *Transaction ID:* ${verificationResult.extractedFields?.transactionId || order.transactionId || 'N/A'}\n` +
        `📊 *Status:* Order Confirmed & Processing\n\n` +
        orderItemsText +
        `\n📄 *Invoice:* I've sent your invoice. Please save it for your records.\n\n` +
        `*What Happens Next:*\n` +
        `• Order processing: 24-48 hours\n` +
        `• Quality check & packaging\n` +
        `• Shipping confirmation with tracking\n` +
...
async function generateAndSendInvoice(order, message) {
    try {
        ...
        try {
            ...
            console.error('❌ PDF invoice failed, sending text invoice:', pdfError.message);
...
async function sendAlreadyVerifiedMessage(message, state) {
    ...
    await message.reply(
        ...
        `💰 Amount Paid: ₹${state.amount}\n` +
        `📦 Status: Processing\n` +
        `🎯 Verification: ${state.confidence}% (${state.engine})\n\n` +
        `_To place a new order, send *!menu*_`
    );
}
...
async function showPaymentInstructions(message, client) {
    ...
    await message.reply(
        ...
        `1. Scan our QR code from the website\n` +
        `2. Pay exact amount\n` +
        `3. Send screenshot\n\n` +
        `*Option 2 - UPI ID:*\n` +
        `1. Open UPI app (GPay, PhonePe, PayTM)\n` +
        `2. Send to: ${primaryUpi}\n` +
        `3. Pay exact amount\n` +
        `4. Send screenshot\n\n` +
        `*Option 3 - Phone Number:*\n` +
        `1. Send to our registered number\n` +
        `2. Pay exact amount\n` +
        `3. Send screenshot\n\n` +
        ...
        `✅ "SUCCESSFUL" status\n` +
        `✅ Transaction ID\n` +
        `✅ Date & Time (within 15 minutes)\n\n` +
        `*We verify automatically in seconds!*`
    );
}
...
async function showPendingVerifications(message, client) {
    try {
        ...
        pendingVerifications.forEach((verification, index) => {
            ...
            response += `   *Phone:* ${verification.customerPhone}\n`;
            response += `   *Amount:* ₹${verification.orderDetails?.totalAmount || 'N/A'}\n`;
            response += `   *Confidence:* ${verification.ocrAnalysis?.confidenceScore || 0}%\n`;
            response += `   *Engine:* ${verification.metadata?.ocrEngine || 'Unknown'}\n`;
            response += `   *Submitted:* ${new Date(verification.createdAt).toLocaleString()}\n`;
            response += `   *Verify:* !verify ${verification.orderNumber}\n`;
            response += `   *Reject:* !reject ${verification.orderNumber} reason\n\n`;
        });
...
async function showVerificationStats(message, client) {
    try {
        ...
        const response = 
            ...
            `📝 Total: ${stats.total || 0}\n\n` +
            `*OCR Performance:*\n` +
            `📈 Avg Confidence: ${Math.round(engineStats.averageConfidence || 0)}%\n` +
            `🔄 Paddle Success: ${engineStats.paddleSuccessRate || '0%'}\n` +
            `🔄 Easy Success: ${engineStats.easySuccessRate || '0%'}`;
...
export function cleanupVerificationState() {
    ...
    // Clear company UPI cache every hour
    for (const [companyId, data] of companyUpiCache.entries()) {
        if (now - data.timestamp > VALIDATION_CONFIG.companyUpiCacheTime) {
            companyUpiCache.delete(companyId);
        }
    }
}
...
export function clearCompanyCache(companyId) {
    if (companyId) {
        companyUpiCache.delete(companyId);
        console.log(`🗑️ Cleared UPI cache for company: ${companyId}`);
    }
}

// Run cleanup every hour
setInterval(cleanupVerificationState, 60 * 60 * 1000);
