...
/**
 * Main booking flow handler - UPDATED with companyId
 */
export async function handleBookingFlow(message, client, userSession, userSessions, companyId = null) {
    const userMessage = message.body.trim();
    const from = message.from;

    try {
        // ✅ Initialize booking data with company context
        if (!userSession.bookingData) {
            userSession.bookingData = {};
        }
        
        // ✅ Store companyId in session for all subsequent steps
        if (companyId) {
            userSession.bookingData.companyId = companyId;
            console.log(`🏢 [BookingFlow] Company context set: ${companyId}`);
        }
        ...
        switch (userSession.bookingState) {
            case 'START_BOOKING':
                return await showServices(message, userSession, companyId);

            case BOOKING_STATES.AWAITING_SERVICE:
                return await handleServiceSelection(message, userSession, companyId);

            case BOOKING_STATES.AWAITING_DATE:
                return await handleDateSelection(message, userSession, companyId);

            case BOOKING_STATES.AWAITING_TIME:
                return await handleTimeSelection(message, userSession, companyId);

            case BOOKING_STATES.AWAITING_CUSTOMER_NAME:
                return await handleCustomerName(message, userSession, companyId);

            case BOOKING_STATES.AWAITING_PHONE:
                return await handlePhoneInput(message, userSession, companyId);

            case BOOKING_STATES.AWAITING_CONFIRMATION:
                return await handleFinalConfirmation(message, userSession, client, companyId);
...
/**
 * Handle "MyBookings" command - View user's bookings with company filter
 */
export async function handleMyBookings(message, client, userSession, companyId = null) {
    try {
        const whatsappId = userSession.whatsappId;
        
        console.log(`📅 [MyBookings] Fetching for WhatsApp: ${whatsappId}, Company: ${companyId || 'all'}`);
        
        await message.reply('🔍 *Fetching your bookings...*');

        // ✅ Get company-specific bookings
        const upcomingBookings = await bookingApiService.getUpcomingClientBookings(whatsappId, companyId);
        const pastBookings = await bookingApiService.getPastClientBookings(whatsappId, companyId);
...
// ========== SIMPLIFIED HELPER FUNCTIONS WITH COMPANY CONTEXT ==========

/**
 * STEP 1: Show all available services for a company (like products)
 */
async function showServices(message, userSession, companyId) {
    await message.reply(`🔍 *Loading available services...*`);

    // ✅ Get company-specific services
    const services = await bookingApiService.getServices({ 
        isActive: true,
        companyId: companyId 
    });
    ...
    // Store services and companyId in session
    userSession.bookingData.services = services;
    userSession.bookingData.companyId = companyId;
...
/**
 * STEP 2: Handle service selection with company validation
 */
async function handleServiceSelection(message, userSession, companyId) {
    const input = message.body.trim();
    const services = userSession.bookingData.services;
    ...
    // ✅ Validate service with company context
    const serviceValidation = await validateService(selectedService._id, companyId);
    ...
    userSession.bookingData.servicePrice = selectedService.basePrice;
    userSession.bookingData.businessId = selectedService.professionalId; // Get business ID from service
    userSession.bookingData.companyId = companyId; // Ensure company context
...
/**
 * STEP 3: Handle date selection with company validation
 */
async function handleDateSelection(message, userSession, companyId) {
    const input = message.body.trim();
    ...
    // ✅ Get available slots for this business with company validation
    const availability = await getAvailableSlots(
        userSession.bookingData.businessId,
        dateStr,
        companyId  // Pass companyId for validation
    );
...
/**
 * STEP 4: Handle time selection with company validation
 */
async function handleTimeSelection(message, userSession, companyId) {
    const input = message.body.trim().toLowerCase();
    ...
    // ✅ Final availability check with company validation
    console.log('🔄 Performing final availability check with API...');
    console.log('📤 checkAvailability params:', {
        businessId: userSession.bookingData.businessId,
        date: userSession.bookingData.date,
 ...
 * STEP 5: Handle customer name
 */
async function handleCustomerName(message, userSession, companyId) {
    const name = message.body.trim();
 ...
 * STEP 6: Handle phone input
 */
async function handlePhoneInput(message, userSession, companyId) {
    const phone = message.body.trim();
...
/**
 * STEP 7: Handle final confirmation with company context
 */
async function handleFinalConfirmation(message, userSession, client, companyId) {
    const input = message.body.trim().toLowerCase();
    ...
    if (input === 'confirm' || input === 'confirm & pay') {
        ...
        // Check if payment required
        if (userSession.bookingData.paymentRequired) {
            // ✅ Save booking with company context and pending status
            const saveResult = await saveBooking(
                userSession, 
                userSession.bookingData, 
                true, // payment required
            ...
            // Show payment instructions
            await showPaymentInstructions(message, userSession, saveResult.booking, companyId);
            // Payment would be handled by separate payment handler
            
        } else {
            // ✅ Save booking with company context and confirmed status
            const saveResult = await saveBooking(
                userSession, 
                userSession.bookingData, 
                false, // no payment required
            ...
            // Send confirmation
            await sendBookingConfirmation(message, userSession, saveResult.booking, companyId);
...
/**
 * Show payment instructions with company context
 */
async function showPaymentInstructions(message, userSession, booking, companyId) {
    // ✅ Get company-specific UPI ID (you would fetch this from company settings)
    const companyUpiId = await getCompanyUpiId(companyId) || 'merchant@okhdfcbank';
...
/**
 * Send booking confirmation with company context
 */
async function sendBookingConfirmation(message, userSession, booking, companyId) {
    const confirmMsg = 
        `✅ *BOOKING CONFIRMED!*\n\n` +
        `Booking #: ${booking.bookingNumber}\n` +
        `Service: ${booking.serviceName}\n` +
...
/**
 * Helper to get company UPI ID (implement based on your company settings)
 */
async function getCompanyUpiId(companyId) {
    try {
        // You would implement this based on your company settings
        // For now, return a default
        return 'merchant@okhdfcbank';
    } catch (error) {
        console.error('❌ Error getting company UPI ID:', error);
        return 'merchant@okhdfcbank';
    }
}