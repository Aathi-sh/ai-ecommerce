...
// handlers/productsHandler.js - PROFESSIONAL MULTI-TENANT VERSION
// Handles product browsing, searching, and ordering with company isolation
// FIXED: Category handling, product display, and API integration
...
function findSimilarProducts(products, searchTerm, companyId) {
    ...
    products.forEach(product => {
        ...
        // ✅ FIXED: Handle category safely - use categoryName if available
        let categoryText = '';
        if (product.categoryName) {
            categoryText = product.categoryName.toLowerCase();
...
function getSearchSuggestions(products, searchTerm, companyId) {
    ...
    products.forEach(product => {
        ...
        // ✅ FIXED: Handle category safely using categoryName
        let categoryText = '';
        if (product.categoryName) {
            categoryText = product.categoryName.toLowerCase();
        } else if (product.category) {
...
/**
 * ENHANCED SMART PRODUCT DISPLAY with company context
 * ✅ FIXED: Proper category handling using categoryName
 */
async function sendProductWithImage(message, client, product, title = '') {
    try {
        console.log(`\n🎯 PROCESSING PRODUCT: ${product.productName}`);
        ...
        // ✅ FIXED: Get category name safely from multiple sources
        let categoryName = 'General';
        if (product.categoryName && typeof product.categoryName === 'string') {
            ...
            if (typeof product.category === 'string') {
                ...
                categoryName = product.category.name || product.category._id || 'General';
            }
        }
        
        // ✅ FIXED: Get subCategory name safely
        let subCategoryName = '';
        if (product.subCategoryName && typeof product.subCategoryName === 'string') {
        ...
        console.log(`🔖 SubCategory: ${subCategoryName || 'None'}`);
        ...
        // ✅ FIXED: Use categoryName variable
        productText += `🏷️ *Category:* ${categoryName}\n`;
        
        // Add subCategory if exists
        if (subCategoryName) {
            productText += `📌 *SubCategory:* ${subCategoryName}\n`;
        }
        
        productText += (product.brand ? `🏭 *Brand:* ${product.brand}\n` : '') +
            `\n📝 *Description:*\n${product.description || 'Premium quality product'}\n`;
...
export async function handleDirectProductSearch(message, client, productName, companyId) {
    ...
    try {
        ...
        if (!allProducts || allProducts.length === 0) {
            ...
            await message.reply(
                `📭 *No Products Available*\n\n` +
                `Sorry, there are currently no products in your catalog.\n\n` +
                `Please check back later or contact support.`
            );
            processingSearches.delete(from);
            return true;
...
/**
 * Enhanced showProductsPage with company context
 * ✅ FIXED: Better handling of empty products
 */
async function showProductsPage(message, client, page = 0, searchTerm = '', companyId = null, customTitle = '', preFilteredProducts = null) {
    try {
        const from = message.from;
        ...
        if (searchTerm) {
            if (preFilteredProducts) {
                ...
                if (!allProducts || allProducts.length === 0) {
                    console.log('❌ No products found in database for this company');
                    await message.reply(
                        `📭 *No Products Available*\n\n` +
                        `Sorry, there are currently no products in your catalog.\n\n` +
                        `Please check back later or contact support.`
        ...
        if (!products || products.length === 0) {
            console.log('❌ No products to display on this page');
            let noProductsMessage;
